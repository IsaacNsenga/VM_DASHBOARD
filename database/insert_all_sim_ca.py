...
sheet_name = "all_sim_ca"  # Feuille à importer
...
try:
    for index, row in df.iterrows():
        ...
        for col in df.columns[1:]:  # Colonnes suivantes = dates
            try:
                date_releve = pd.to_datetime(col).date()  # Convertir l’en-tête en date
                total_sim = row[col]  # Valeur dans la cellule
                
                if pd.isna(total_sim):
                    continue  # Ignorer les valeurs vides
                
                sql = """
                    INSERT INTO all_sim_ca (numero_mpos, date_releve, total_sim)
                    VALUES (%s, %s, %s)
                ...
                print(f"❌ Erreur à la ligne {index}, colonne {col} : {e}")
